...
local HolyTears = RegisterMod("Holy Tears Mod",1)
local Item = Isaac.GetItemIdByName("Holy Tears")

function HolyTears:MC_EVALUATE_CACHE(player,cacheFlag)--evaluate cache
local player = Isaac.GetPlayer(0)--get player data
 if cacheFlag == CacheFlag.CACHE_TEARFLAG  then--when updating player.TearFlags...
  if player:HasCollectible(Item) then--if the player has our item...
   player.TearFlags = player.TearFlags + TearFlags.FLAG_HOLY_LIGHT
   tearflag = true--add flag 12
   --(Ipecac) and flag 4 (Poison) to the player's TearFlags
   --player.TearFallingAcceleration = player.TearFallingAcceleration -1;
end
end
end
  
  HolyTears:AddCallback(ModCallbacks.MC_EVALUATE_CACHE,HolyTears.MC_EVALUATE_CACHE)
...
local function modUpdateCheck(_mod)
  player = Isaac.GetPlayer(0)
  if player:HasCollectible(Isaac.GetItemIdByName("Lil Demigod")) and demiExists == false then
    Isaac.Spawn(EntityType.ENTITY_FAMILIAR, 121, 0, Vector (player.Position.X,player.Position.Y), Vector (0,0), player)
    demiExists = true
  end
 end
...
local Elements = RegisterMod("Fire Element",1)
local Element = Isaac.GetItemIdByName("Elemental Fire")


function Elements:MC_EVALUATE_CACHE(player,cacheFlag)--evaluate cache
local player = Isaac.GetPlayer(0)--get player data
 if cacheFlag == CacheFlag.CACHE_TEARFLAG  then--when updating player.TearFlags...
  if player:HasCollectible(Element) then--if the player has our item...
   player.TearFlags = player.TearFlags + TearFlags.FLAG_FIRE
   player:GetEffects():AddCollectibleEffect(257,false)
   tearflag = true--add flag 12
   --(Ipecac) and flag 4 (Poison) to the player's TearFlags
   --player.TearFallingAcceleration = player.TearFallingAcceleration -1;
end
...
function Elements:update()
  local player = Isaac.GetPlayer(0)
  local pos = Isaac.GetFreeNearPosition(Vector (player.Position.X,player.Position.Y),2)
  if player:HasCollectible(Element) then
    if tearflag == true then
        
      local sound_entity = Isaac.Spawn(EntityType.ENTITY_FLY, 0, 0, Vector(320,300), Vector(0,0), nil):ToNPC()
sound_entity:PlaySound(SoundEffect.SOUND_1UP, 40, 60, false, 1)
sound_entity:Remove()
    if player:IsDead() then
      player:Revive()
      player:AddSoulHearts(2)
      Isaac.GridSpawn(GridEntityType.GRID_FIREPLACE,2,Vector (pos),Vector (0,0),player)
...
Elements:AddCallback(ModCallbacks.MC_POST_UPDATE,Elements.update,EntityType.ENTITY_PLAYER)
Elements:AddCallback(ModCallbacks.MC_EVALUATE_CACHE,Elements.MC_EVALUATE_CACHE)

local Elements22 = RegisterMod("Thunder Element",1)
local Element2 = Isaac.GetItemIdByName("Elemental Thunder")

function Elements22:update()
  local player = Isaac.GetPlayer(0)
  local pos = Isaac.GetFreeNearPosition(Vector (player.Position.X,player.Position.Y),2)
  if player:HasCollectible(Element2) then
    player:AddCostume("elementalthunder.anm2",false)
    if player:IsDead() then
      player:Revive()
      player:AddSoulHearts(2)
      Isaac.GridSpawn(GridEntityType.GRID_SPIKES,2,Vector (pos),Vector (0,0),player)
    end
  end
end

function Elements22:MC_EVALUATE_CACHE(player,cacheFlag)--evaluate cache
local player = Isaac.GetPlayer(0)--get player data
 if cacheFlag == CacheFlag.CACHE_TEARFLAG  then--when updating player.TearFlags...
  if player:HasCollectible(Element2) then--if the player has our item...
   player.TearFlags = player.TearFlags + TearFlags.FLAG_JACOBS_LADDER
   player.TearColor = Color(0,60,233,1,1,0,0)
   tearflag = true--add flag 12
   --(Ipecac) and flag 4 (Poison) to the player's TearFlags
   --player.TearFallingAcceleration = player.TearFallingAcceleration -1;
end
...
Elements22:AddCallback(ModCallbacks.MC_POST_UPDATE,Elements22.update,EntityType.ENTITY_PLAYER)
Elements22:AddCallback(ModCallbacks.MC_EVALUATE_CACHE,Elements22.MC_EVALUATE_CACHE)

local   trinity = RegisterMod( "Trinity", 1);
local   trinity_item = Isaac.GetItemIdByName( "Trinity" )

function trinity:use_trinityeffect()
    local   player = Isaac.GetPlayer(0);
		if player:GetNumCoins() < 1 then
		player:AnimateHappy();
        player:GetEffects():AddCollectibleEffect(2, false);
        player:GetEffects():AddCollectibleEffect(1, false);
		player:GetEffects():AddCollectibleEffect(213, false);
		player:GetEffects():AddCollectibleEffect(374, false);
        player:AddCoins(-25);
    else
        player:AnimateSad();
    end
...
trinity:AddCallback(ModCallbacks.MC_USE_ITEM, trinity.use_trinityeffect, trinity_item )

local jesus = RegisterMod("Jesus",1)
local wine = Isaac.GetItemIdByName("Jesus's Wine")
lucky = false

function jesus:cacheupdate(player,cacheFlag)
  local player = Isaac.GetPlayer(0)
  if cacheFlag == CacheFlag.CACHE_DAMAGE and damage == true then
    player.Damage = player.Damage +1;
  end
  if cacheFlag == CacheFlag.CACHE_LUCK and lucky == false then
    player.Luck = player.Luck +2;
    lucky = true
end    
end
function jesus:update()
  local player = Isaac.GetPlayer(0)
  local jesusplayer = player:GetName()
  local chance = math.random(1)
  local damage = false
  local lucky = false
   if player:GetName() == "Jesus" then 
    if (Game():GetFrameCount() == 1) then 
      player:AddCollectible(Isaac.GetItemIdByName("Jesus's Wine"), 1, false); player:ClearCostumes(); 
    end
    end
    if player:HasCollectible(wine) then
      player.TearColor = Color(0.8,0,100,1,0,0,0)
      damage = true
    end
  end
...
function jesus:render()
  if lucky == true then
  Isaac.RenderText("JESUSWINETRUE",30,210,255,0,0,1,255) 
  elseif lucky == false then
  Isaac.RenderText("JESUSWINEFALSE",30,210,255,0,0,1,255) 
  if synergy1 == true then
   Isaac.RenderText("SYNERGY1TRUE",30,240,255,0,0,1,255)  
 elseif synergy1 == false then
   Isaac.RenderText("SYNERGY1FALSE",30,240,255,0,0,1,255) 
end
end
end
jesus:AddCallback(ModCallbacks.MC_POST_UPDATE,jesus.update,EntityType.ENTITY_PLAYER)
jesus:AddCallback(ModCallbacks.MC_EVALUATE_CACHE,jesus.cacheupdate)
jesus:AddCallback(ModCallbacks.MC_POST_RENDER,jesus.render)

local synergy = RegisterMod("Synergies",1)

local fireTear_id = 65

function synergy:update()
  synergy1 = false
local player = Isaac.GetPlayer(0)
if player:HasCollectible(Isaac.GetItemIdByName("Jesus's Wine")) then
  if player:HasCollectible(CollectibleType.COLLECTIBLE_BALL_OF_TAR) then
  synergy:Brimstone()
  synergy1 = true
elseif player:HasCollectible(CollectibleType.COLLECTIBLE_SPEAR_OF_DESTINY) then
  synergy:Spear()
  synergy1 = true
end
end
synergy:AddCallback(ModCallbacks.MC_POST_UPDATE,synergy.update,EntityType.ENTITY_PLAYER)


function synergy:Brimstone()
player.TearColor:Color(0.8,0,100,1,0,0,0)
end
end
synergy:AddCallback(ModCallbacks.MC_POST_UPDATE,synergy.Brimstone,EntityType.ENTITY_PLAYER)